    ...
    reader.onload = (e) => {
      try {
        ...
        if (file.name.endsWith('.csv')) {
          ...
          console.log('✓ CSV data file loaded successfully:', manualData.value.length, 'records');
        } else {
          // JSON格式
          manualData.value = JSON.parse(content);
          console.log('✓ JSON data file loaded successfully');
        }
      } catch (err) {
...
const parseCSVData = (content: string, type: 'track' | 'vital'): any[] => {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    try {
      ...
      console.warn('CSV line parsing failed:', line, err);
    }
  }
...
const applyConfigAndPlay = (layout: any, data: any) => {
  ...
  if (layout && layout.objects) {
    ...
    console.log('📥 Layout applied:', layout.objects.length, 'objects');
  }
  ...
  if (data[0].persons) {
    console.log('✅ Track data loaded, start playback', data.length, 'frames');
    ...
    // Vital数据
    console.log('✅ Vital data loaded', data.length, 'records');
    // TODO: 实现vital数据播放
  }
  
  console.log('✅ Config applied, start playback', {
