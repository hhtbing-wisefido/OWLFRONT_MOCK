...
import type { PostureIconConfig, BaseObject } from '@/utils/radar/types';
import { 
  POSTURE_CONFIGS,
  VITAL_SIGN_CONFIGS, 
  getHeartRateStatus, 
  getBreathingStatus, 
  getSleepStatus 
} from '@/utils/radar/postureIcons';
import { drawObjects } from '@/utils/radar/drawObjects';
import { drawLine, drawRectangle, drawCircle, drawSector } from '@/utils/radar/drawShapes';
import type { Point, RadarPoint } from '@/utils/radar/types';
import { getRadarBoundaryVertices, toCanvasCoordinate } from '@/utils/radar/radarUtils';
import { RADAR_DEFAULT_CONFIG, MOVE_STEP, FURNITURE_CONFIGS, type FurnitureType, PersonPosture } from '@/utils/radar/types';
...
const preloadPostureIcons = async (): Promise<void> => {
  console.log('🔄 Start preloading posture icons...');
  ...
  try {
    ...
    console.log(`✅ Posture icons preloaded, ${postureImageCache.size} icons in cache`);
  } catch (e) {
    console.error('❌ Posture icon preload error:', e);
  }
};
...
const handleMouseDown = (event: MouseEvent) => {
  ...
  if (selectedObj) {
    ...
    if (cpCheck.hit) {
      ...
      console.log('🎯 Start dragging control point:', cpCheck.point.type);
      return;
    }
    ...
    if (isPointInObject(logicalX, logicalY, selectedObj)) {
      ...
      draggedObject.value = selectedObj;
      cursorStyle.value = 'grabbing';
      console.log('🎯 Start dragging object:', selectedObj.name || selectedObj.typeName);
      return;
    }
  }
};
...
const createObjectFromDrawing = (startX: number, startY: number, endX: number, endY: number): string | null => {
  ...
  console.log('📍 Create object coordinates (logical):', { 
    start: { x: startX, y: startY }, 
    end: { x: endX, y: endY },
  ...
  switch (canvasStore.drawingMode) {
    case 'line':
      ...
      if (lineLength < 5) {
        console.log('⚠️ Line length too small (minimum 5), creation cancelled');
        return null;
      }
  ...
  if (newObject) {
    ...
    console.log('✅ Object created:', newObject.name);
...
const handleCanvasClick = (event: MouseEvent) => {
  ...
  if (canvasStore.pendingObjectType && !canvasStore.drawingMode) {
    ...
    console.log(`✅ Device created at (${logicalX}, ${logicalY}): ${type}`);
    return;
  }
...
const handleCanvasDblClick = (_event: MouseEvent) => {
  ...
  console.log('🔄 Double-click canvas, reset all selected states');
};
...
const drawPersons = (ctx: CanvasRenderingContext2D) => {
  ...
  persons.forEach((person, i) => {
    ...
    const personKey = `${person.deviceCode}_${person.personIndex}`;  // 不包含 posture，以便同一个人从非跌倒状态变成跌倒状态时可以再次触发报警
    if (person.posture === PersonPosture.FallConfirm) {
      ...
      // 如果姿态不是跌倒，清除该人员的报警记录（允许下次跌倒时再次报警）
      fallAlarmSet.delete(personKey);
    }
    ...
    if (i === 0) {
      console.log(`📍 Person coordinate conversion:`, {
        radarCoordinates: `(H=${radarPoint.h}, V=${radarPoint.v})`,
        canvasCoordinates: `(${canvasPoint.x.toFixed(1)}, ${canvasPoint.y.toFixed(1)})`,
        screenCoordinates: `(${screenX.toFixed(1)}, ${screenY.toFixed(1)})`
      });
    }
    ...
    if (i === 0 || !postureImg || !postureImg.complete) {
      ...
      console[logLevel](`${i === 0 ? '📍' : '⚠️'} Posture icon posture=${displayPosture}:`, {
    ...
    // 床上相关姿态（SitUpBed、SitUpBedSuspect、SitUpBedConfirm、Lying）不显示轨迹点
    const bedPostures = [
      PersonPosture.Lying,           // 6
      PersonPosture.SitUpBed,        // 9
      PersonPosture.SitUpBedSuspect, // 10
      PersonPosture.SitUpBedConfirm  // 11
    ];
    if (person.deviceCode && person.personIndex !== undefined && !bedPostures.includes(person.posture)) {
...
const drawPersonTrajectory = (
  ...
  // 🔍 调试轨迹数量
  if (personIndex === 0) {
    console.log(`🔍 Draw trajectory (person ${personIndex}, moving=${isMoving}):`, {
  ...
  // 每个人的专属颜色（绿/黄/蓝/红）
  const personColors = [
    { name: 'Green', r: 80, g: 220, b: 80 },    // 人0: 绿色
    { name: 'Yellow', r: 255, g: 220, b: 0 },    // 人1: 黄色
    { name: 'Blue', r: 80, g: 150, b: 255 },   // 人2: 蓝色
    { name: 'Red', r: 255, g: 80, b: 80 }     // 人3: 红色
  ];
