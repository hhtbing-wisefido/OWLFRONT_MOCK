...
const drawWaveform = () => {
  ...
  // 调试信息
  if (props.mode === 'realtime' && props.data.length > 0) {
    console.log(`🎨 Draw waveform: data points=${props.data.length}, latest time=${props.data[props.data.length - 1].timestamp.toFixed(1)}s`);
  }
  ...
  // 计算边距
  const padding = { left: 35, right: 10, top: 10, bottom: 80 };  // 增加 bottom 从 60 到 80，确保 X 轴与容器底框有足够间距
...
const handleMouseMove = (e: MouseEvent) => {
  ...
  const padding = { left: 35, right: 10, top: 10, bottom: 80 };  // 与绘制函数保持一致
  const chartWidth = canvasWidth.value - padding.left - padding.right;
...
.hrrr-waveform {
  position: relative;
  border-radius: 4px;
  padding: 8px 8px 20px 8px;  /* 增加 padding-bottom 从 8px 到 20px，确保 X 轴与容器底框有足够间距 */
