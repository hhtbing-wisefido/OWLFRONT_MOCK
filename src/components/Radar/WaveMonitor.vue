...
import { MockRadarService } from '@/utils/radar/mockRadarData';
...
const handleLoadVitalFile = () => {
  ...
  if (vitalParsedData.value.length > 0) {
    ...
    console.log(`✅ Load Vital historical data: ${vitalWaveformData.value.length} records`);
  } else {
...
const handleRealTimeVital = () => {
  ...
  console.log('✅ Switched to real-time mode, start receiving radar data');
  console.log('   Tip: If no data, please click Demo button to start simulated data');
};
...
const startPlayback = async (source: 'backend' | 'file' | 'demo') => {
  try {
    ...
    if (!displayRadarId.value && canvasRadars.value.length > 0) {
      displayRadarId.value = canvasRadars.value[0].id;
      console.log(`🎯 Auto-select display radar: ${canvasRadars.value[0].name}`);
    }
    ...
    if (source === 'backend') {
      ...
      if (bed) {
        ...
        console.log('⚠️ Bed object not detected, will use radar center area to simulate on-bed scenario');
      }
    ...
    console.log('✅ Start playing historical data...');
...
const parseRealData = (content: string): any[] => {
  ...
  console.log(`📂 Parsing file, ${lines.length} lines total`);
