...
import { FURNITURE_CONFIGS } from '@/utils/radar/types';
import { RADAR_DEFAULT_CONFIG } from '@/utils/radar/types';
import {
  convertInstallModel,
  convertHeight,
  convertBoundary,
...
watch(
  ...
  (obj) => {
    ...
    if (obj.device?.category === 'iot') {
      ...
      activeFurniture.value = null;
      activeTool.value = null;
      console.log('🔵 Highlight device button:', typeName);
    } 
    // 检查是否是家具/结构
      ...
      if (obj.visual?.color) {
        ...
        console.log('🎨 Sync object color:', obj.visual.color);
      }
      console.log('🔵 Highlight furniture button:', typeName);
    }
  },
  { deep: true }
);
...
const selectDrawTool = (tool: 'line' | 'rect' | 'sector' | 'circle') => {
  ...
  if (!activeFurniture.value) {
    console.log('⚠️ Please select furniture first, then select drawing tool');
    return;
  }
  ...
  if (activeTool.value === tool) {
    ...
    console.log('🎨 Drawing tool activated:', tool, 'Furniture type:', activeFurniture.value);
  }
};
...
const toggleTransparent = (event: Event) => {
  ...
  if (selectedObj && selectedObj.device?.category !== 'iot') {
    ...
    console.log('🎨 Toggle transparent mode:', selectedObj.name || selectedObj.typeName, checked ? 'border only' : 'filled');
  }
};
...
const toggleLock = (event: Event) => {
  ...
  if (selectedObj) {
    ...
    console.log(checked ? '🔒 Object locked' : '🔓 Object unlocked');
  }
};
...
const selectColor = (color: keyof typeof colorMap) => {
  // 必须先选中家具才能使用颜色选择
  if (!activeFurniture.value) {
    console.log('⚠️ Please select furniture first, then select color');
    return;
  }
  ...
  if (selectedObj && selectedObj.device?.category !== 'iot') {
    ...
    console.log('🎨 Update object color:', selectedObj.name || selectedObj.typeName, '→', newColor);
  }
};
...
const selectFurniture = (type: string) => {
  ...
  if (furnitureConfig) {
    activeColor.value = furnitureConfig.color;
    console.log(`🪑 Selected furniture type: ${type}, activate drawing tool: ${tool}, use default color: ${furnitureConfig.color}`);
  } else {
    // 如果没有预设，使用gray作为默认颜色
    if (!activeColor.value) {
      activeColor.value = colorMap.gray;
    }
    console.log(`🪑 Selected furniture type: ${type}, activate drawing tool: ${tool}`);
  }
};
...
const addDevice = (type: string) => {
  ...
  // 不直接创建，而是设置待放置状态
  canvasStore.setPendingObjectType(type);
  console.log(`📍 Ready to place device: ${type}, please click on canvas to place`);
  ...
  console.log(`✅ Device created: ${type}`, newDevice);
  */
};
...
const updateDeviceProp = (prop: string, value: any) => {
  ...
  if (prop === 'installModel' && isRadarDevice.value) {
    ...
    if (defaultConfig) {
      ...
      console.log(`🔧 Switch radar mode to ${value}, default config applied (including rotation angle ${defaultConfig.Rotation}°):`, defaultConfig);
      return;
    }
  }
  ...
  console.log(`🔧 Update device property ${prop}:`, value);
    return;
  }
  ...
  if (prop === 'installModel') {
    ...
    console.log(`🔧 Template: Switch install mode to ${value}`);
  }
};
...
const shiftAllObjectsLeft = () => {
  ...
  console.log(`📦 Move all left by 10px, moved ${movedCount}/${allObjects.length} objects`);
};
...
const moveObject = (dx: number, dy: number) => {
  ...
  if (selectedObject.value.interactive?.locked) {
    console.log('⚠️ Object locked, cannot move');
    return;
  }
  ...
  console.log(`📦 Move object ${obj.name} by (${dx}, ${dy})`);
};
...
const rotateObject = (angleDelta: number) => {
  ...
  // 检查是否锁定
  if (selectedObject.value.interactive?.locked) {
    console.log('⚠️ Object locked, cannot rotate');
    return;
  }
  ...
  console.log(`🔄 Rotate object ${selectedObject.value.name} to ${newAngle}° (counterclockwise)`);
...
const queryDevice = async () => {
  ...
  try {
    ...
    console.log('📖 Query radar config success:', mockData);
  } catch (error) {
...
const saveConfig = async () => {
  ...
  try {
    ...
    if (!baseline || !baseline.queriedAt) {
      console.warn('⚠️ Baseline config not found, please execute Query first');
      alert('Please execute Query operation first to get device current config');
      return;
    }
    ...
    if (commands.length === 0) {
      console.log('✅ Config unchanged, no need to write');
      alert('Config unchanged, no need to save');
      return;
    }
    
    // 4. 显示待执行的命令
    console.log('📝 Command list to execute:', commands);
    const commandSummary = commands.map((cmd, idx) => 
      `${idx + 1}. ${cmd.description}`
    ).join('\n');
    
    const confirmed = confirm(
      `The following config changes detected, confirm to write to device?\n\n${commandSummary}\n\nTotal ${commands.length} commands`
    );
    ...
    if (executeResult.allSuccess) {
      ...
      if (needManualVerify) {
        console.log('🔍 Execute manual verification (optional)...');
        await new Promise(resolve => setTimeout(resolve, 2000));
        const verifyResult = await verifyRadarConfig(device.device?.iot?.deviceId, currentConfig);
        
        if (!verifyResult.success) {
          console.warn('⚠️ Manual verification found issues, but server confirmed write');
          const failedItems = verifyResult.failures.map(f => `  - ${f}`).join('\n');
          alert(`Config written to device, but manual verification found differences:\n\n${failedItems}`);
        }
      }
      ...
      alert('Config saved successfully! All commands executed.');
      ...
      // 执行手动Query，获取设备真实状态
      console.log('🔍 Execute manual Query, get device current status...');
      await new Promise(resolve => setTimeout(resolve, 2000));
      ...
      alert(`Some config writes failed:\n\n${failedCommands}\n\nBaseline updated to device current state.`);
    }
...
const generateConfigCommands = (
  ...
  if (baseline.installModel !== current.installModel) {
    commands.push({
      type: 'installModel',
      description: `Install mode: ${baseline.installModel} → ${current.installModel}`,
  ...
  if (Math.abs((baseline.height || 0) - (current.height || 0)) > 1) {
    commands.push({
      type: 'height',
      description: `Install height: ${baseline.height}cm → ${current.height}cm`,
  ...
  if (boundaryChanged) {
    ...
    commands.push({
      type: 'boundary',
      description: `Boundary range: L${b.leftH}/R${b.rightH}/F${b.frontV}/B${b.rearV} → L${c.leftH}/R${c.rightH}/F${c.frontV}/B${c.rearV}`,
  ...
  baselineAreas.forEach((area: any) => {
    ...
    if (!exists) {
      commands.push({
        type: 'area_delete',
        description: `Delete area: Area ${area.areaId}`,
  ...
  currentAreas.forEach((area: any) => {
    ...
    if (!exists) {
      ...
      commands.push({
        type: 'area_add',
        description: `Add area: ${area.areaName || `Area ${area.areaId}`}`,
      ...
      commands.push({
        type: 'area_update',
        description: `Update area: ${area.areaName || `Area ${area.areaId}`}`,
        mqttKeyValues: convertArea(area),
        deviceId,
...
const verifyRadarConfig = async (deviceId: string, expectedConfig: any): Promise<VerifyResult> => {
  ...
  try {
    ...
    console.log('   Reading config from device...');
    ...
    if (actualConfig.installModel !== expectedConfig.installModel) {
      failures.push(`Install mode mismatch: expected ${expectedConfig.installModel}, actual ${actualConfig.installModel}`);
    }
    
    // 3. 对比 height（允许±2cm误差）
    const heightDiff = Math.abs(actualConfig.height - expectedConfig.height);
    if (heightDiff > 2) {
      failures.push(`Install height mismatch: expected ${expectedConfig.height}cm, actual ${actualConfig.height}cm`);
    }
    
    // 4. 对比 boundary
    if (actualConfig.boundary.leftH !== expectedConfig.boundary.leftH) {
      failures.push(`Left boundary mismatch: expected ${expectedConfig.boundary.leftH}, actual ${actualConfig.boundary.leftH}`);
    }
    if (actualConfig.boundary.rightH !== expectedConfig.boundary.rightH) {
      failures.push(`Right boundary mismatch: expected ${expectedConfig.boundary.rightH}, actual ${actualConfig.boundary.rightH}`);
    }
    if (actualConfig.boundary.frontV !== expectedConfig.boundary.frontV) {
      failures.push(`Front boundary mismatch: expected ${expectedConfig.boundary.frontV}, actual ${actualConfig.boundary.frontV}`);
    }
    if (actualConfig.boundary.rearV !== expectedConfig.boundary.rearV) {
      failures.push(`Rear boundary mismatch: expected ${expectedConfig.boundary.rearV}, actual ${actualConfig.boundary.rearV}`);
    }
    ...
    if (expectedAreas.length !== actualAreas.length) {
      failures.push(`Area count mismatch: expected ${expectedAreas.length}, actual ${actualAreas.length}`);
    } else {
      // 检查每个区域是否存在
      expectedAreas.forEach((expected: any) => {
        const actual = actualAreas.find((a: any) => a.areaId === expected.areaId);
        if (!actual) {
          failures.push(`Area ${expected.areaId} not found`);
        } else if (actual.enable !== expected.enable) {
          failures.push(`Area ${expected.areaId} enable status mismatch`);
        }
      });
    }
    ...
    if (failures.length === 0) {
      console.log('   ✅ All config items verified');
    } else {
      console.warn('   ⚠️ Found config mismatches:');
    ...
    return {
      ...
      failures: [`Verification process error: ${error}`]
    };
  }
};
...
const QueryRadar = async (deviceId: string) => {
  console.log('   📡 Query device config:', deviceId);
  ...
  if (mqttCallbacks.queryDevice) {
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log('   ✅ Query completed:', config);
        return config;
      } else {
    ...
    console.log('   ✅ Query completed (mock):', config);
    return config;
  }
};
...
const executeRadarCommands = async (commands: RadarCommand[]): Promise<ExecuteResult> => {
  console.log('🚀 Start executing radar config commands...');
  console.log('');
  ...
  for (let i = 0; i < commands.length; i++) {
    ...
    try {
      ...
      if (response.success) {
        ...
        console.error(`❌ Command execution failed: ${errorMsg}`);
        failures.push({
          command: cmd,
  ...
  console.log('');
  if (failures.length === 0) {
    console.log(`✅ All commands completed (${successCount}/${commands.length})`);
  } else {
    console.warn(`⚠️ Commands completed, ${failures.length} failed (${successCount}/${commands.length})`);
  }
...
const WriteRadar = async (command: RadarCommand): Promise<ExternalResponse> => {
  ...
  console.log(`   Config data:`, commandData);
...
const calibrate = () => {
  if (!selectedObject.value || !isRadarDevice.value) return;
  console.log('🔧 Execute correction');
  // TODO: 实现校正逻辑
};
...
const layoutSave = async () => {
  ...
  if (!canvasId) {
    console.warn('⚠️ Cannot save: CanvasID not set');
    alert('Cannot save: CanvasID missing');
    return;
  }
...
const layoutExport = () => {
  ...
  console.log(`📤 Layout exported: ${canvasName}`);
};
  ...
  input.onchange = (e: Event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log('📥 Layout imported:', canvasData.params);
        alert(`✅ Layout imported: ${canvasData.objects.length} objects`);
      } catch (error) {
...
const showBindDeviceList = () => {
  ...
  if (devices.length === 0) {
    alert('⚠️ No devices available to bind (all devices are already bound)');
    return;
  }
  
  // 构建选择列表
  let message = 'Please select device to bind:\n\n';
  devices.forEach((device, index) => {
    message += `${index + 1}. ${device.deviceName} (${device.deviceId})\n`;
    if (device.bedId) {
      message += `   Associated bed: ${device.bedName || device.bedId}\n`;
    }
  });
  
  const choice = prompt(message + '\nEnter number (1-' + devices.length + '):');
...
const bindDevice = (device: any) => {
  ...
  console.log(`✅ Device bound: ${device.deviceName} (${device.deviceId})`);
  alert(`✅ Device bound: ${device.deviceName}`);
};
...
const unBindDevice = () => {
  ...
  console.log(`🔓 Device unbound: ${oldDeviceName} → ${tempDevice.deviceName}`);
  alert(`🔓 Device unbound: ${oldDeviceName}\nNew temporary ID: ${tempDevice.deviceName}`);
};

// 切换设置显示
const toggleSettings = () => {
  showSettings.value = !showSettings.value;
  console.log('⚙️ Settings panel:', showSettings.value ? 'show' : 'hide');
};
</script>
