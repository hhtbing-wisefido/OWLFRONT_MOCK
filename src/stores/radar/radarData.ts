...
import type { RadarPoint, PersonData, Point } from '@/utils/radar/types';
...
export const useRadarDataStore = defineStore('radarData', {
  ...
  actions: {
    ...
    setActiveRadar(radarId: string) {
      ...
      console.log(`📡 Activate radar: ${radarId}`);
    },
    ...
    clearDeviceData(deviceCode: string) {
      ...
      console.log(`🧹 Clear radar ${deviceCode} data`);
    },
    ...
    removeInactivePersons() {
      ...
      if (inactiveKeys.length > 0) {
        console.log(`🧹 Remove ${inactiveKeys.length} inactive persons`);
      }
    },
    ...
    clearAllData() {
      this.persons = [];
      this.trajectories = {};
      console.log('🧹 All persons and trajectory data cleared');
    }
  }
});
