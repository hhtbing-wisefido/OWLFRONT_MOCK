...
import type { BaseObject } from '@/utils/radar/types';
import { FURNITURE_CONFIGS } from '@/utils/radar/types';
import { updateRadarAreas } from '@/utils/radar/radarUtils';
...
export const useObjectsStore = defineStore('objects', {
  ...
  actions: {
    ...
    addObject(obj: BaseObject) {
      ...
      console.log(`✅ Add object: ${obj.typeName} (${obj.id})`);
    ...
    removeObject(id: string) {
      ...
      if (index !== -1) {
        ...
        console.log(`🗑️ Remove object: ${obj.typeName} (${id})`);
    ...
    updateObject(id: string, updates: Partial<BaseObject>) {
      ...
      if (obj) {
        ...
        if (shouldUpdate) {
          console.log('🔄 Trigger area update');
          this.updateAllRadarAreas();
        }
      }
    },
    ...
    updateAllRadarAreas() {
      ...
      console.log(`\n🔄 Update all radar areas (${radars.length} radars)`);
    ...
    loadCanvas(canvasKey: string): boolean {
      try {
        ...
        if (!data) {
          console.log(`📭 Canvas not found: ${canvasKey}, using empty layout`);
          this.objects = [];
          this.selectedId = null;
          return false;
        }
        ...
        console.log(`📂 Canvas loaded: ${canvasKey}, ${this.objects.length} objects`);
