...
export function getCanvasParams(): CanvasParams | null {
  ...
  if (params && params.canvasId && params.devices && params.devices.length > 0) {
    console.log('🔗 Received Canvas parameters:', params);
    console.log(`   - CanvasID: ${params.canvasId}`);
    console.log(`   - Device count: ${params.devices.length}`);
    return params;
  }
  
  console.warn('⚠️ No parameters received, using default empty Canvas');
  return null;
}
