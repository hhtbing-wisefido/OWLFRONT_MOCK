...
export function formatDrawResult(result: DrawResult): string {
  switch (result.type) {
    case 'rectangle':
      return `Rectangle:
  Vertices: top-right${pointStr(result.vertices[0])}, top-left${pointStr(result.vertices[1])}, bottom-right${pointStr(result.vertices[2])}, bottom-left${pointStr(result.vertices[3])}
  Width: ${result.width.toFixed(2)}px
  Height: ${result.height.toFixed(2)}px
  Center: ${pointStr(result.center)}`;

    case 'circle':
      return `Circle:
  Center: ${pointStr(result.center)}
  Radius: ${result.radius.toFixed(2)}px`;

    case 'line':
      return `Line:
  Start: ${pointStr(result.start)}
  End: ${pointStr(result.end)}
  Length: ${result.length.toFixed(2)}px
  Center: ${pointStr(result.center)}`;

    case 'sector':
      return `Sector:
  Center: ${pointStr(result.center)}
  Left boundary point: ${pointStr(result.leftPoint)}
  Right boundary point: ${pointStr(result.rightPoint)}
  Radius: ${result.radius.toFixed(2)}px
  Radians: ${result.angle.toFixed(4)} rad
