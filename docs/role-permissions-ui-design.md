...
- 每个展开区域有独立的"保存"按钮
- 点击"保存"按钮，提交该类别的所有权限修改
- 显示修改摘要（修改了多少条权限）

#### 5. 导出功能

- 每个展开区域有独立的"导出"按钮
- 导出该类别的权限矩阵为 Excel 文件
...
interface RoleCategory {
  categoryCode: 'manage' | 'co' | 'ns' | 'cg' | 'it'
  categoryName: string // "管理层" / "合规官" / "护士" / "护工" / "IT支持"
  categoryNameEn: string // "Manage" / "CO" / "NS" / "CG" / "IT"
  roles: string[] // 包含的角色列表
  mainPermissions: string[] // 主要权限描述（3-5条）
  permissions: RolePermission[] // 完整的权限列表
}
```
...
interface PermissionMatrix {
  categoryCode: string // 'manage' | 'co' | 'ns' | 'cg' | 'it'
  resources: string[] // 所有资源类型
  roles: string[] // 该类别包含的角色
  matrix: {
    [resource: string]: {
      [role: string]: {
        permission_type: 'M' | 'R' | 'C' | 'E' | 'D' | '-'
        scope: 'all' | 'assigned_only' | 'location_tag'
      }
    }
  }
}
