...
- **注意**：必须同时输入账号和密码才触发搜索（用于区分相同 user_account 但不同密码的账号）
...
- **安全说明**：
  - **前端只传输 hash，不传输原始 PHI**（phone/email/user_account/password）
  - 传输通过 HTTPS 加密
...
- **多个机构匹配**
  - **Email/Phone 账号**：
    ...
    - 机构列表显示机构名称，可选显示地址信息
  - **UserAccount（用户名）账号**：
    - **安全机制**：不显示机构列表（防止恶意用户选择不属于自己的机构）
    - 机构输入框变为可编辑状态
    - 提示用户手动输入机构名称
    - 实现实时前缀匹配（不区分大小写）：
      - 用户输入机构名称的前几个字符
      - 系统根据输入的前缀实时匹配机构列表
      - 当只有 1 个机构匹配时：自动填充该机构的完整名称并自动选择
      - 当有多个机构匹配时：不弹出下拉列表，继续等待用户输入更多字符以区分
      - 当用户停止输入（blur）时：
        - 如果输入不匹配任何机构 → 提示"机构名错误"
        - 如果输入匹配多个机构 → 提示"请输入更多字符以区分机构"
...
2. **机构搜索**
   - 账号输入时调用搜索 API
   - 处理搜索结果（0/1/多个）
   - **账号类型检测**：自动识别账号类型（Email/Phone/UserAccount）
   - **机构选择处理**：
     - Email/Phone：显示机构选择列表
     - UserAccount 且匹配到多个机构：要求手动输入机构名称，实现实时前缀匹配
...
1. **机构搜索 API**
   - 接收账号和用户类型
   - 识别账号格式（email/phone/userAccount）
   - 根据账号格式计算对应的 hash：
     - Email 格式 → 使用 `email_hash` 字段匹配
     - Phone 格式 → 使用 `phone_hash` 字段匹配
     - UserAccount 格式 → 使用 `user_account_hash` 字段匹配
...
**Staff 用户：**
- 查询 `users` 表
- 匹配条件：`email_hash`、`phone_hash` 或 `user_account_hash`
  - `email_hash`：当输入是 Email 格式时使用
  - `phone_hash`：当输入是 Phone 格式时使用
  - `user_account_hash`：当输入是 UserAccount（用户名）格式时使用
- 关联 `tenants` 表获取机构信息
- 只返回 `status = 'active'` 的机构
- **注意**：`user_account` 字段在数据库中存储为小写（CHECK 约束），前端计算 hash 时也需要转换为小写
...
1. **密码传输安全（HIPAA 合规要求）**
   ...
   - **前端只传输 hash，不传输原始 PHI**：
     - 账号（phone/email/user_account）：传输 `accountHash`（SHA-256 hash）
...
2. **防止恶意选择机构（UserAccount 安全机制）**
   - **问题场景**：当多个机构中存在相同的 `userAccount + password` 组合时，hash 相同，系统会返回所有匹配的机构
   - **安全风险**：如果直接显示机构列表让用户选择，恶意用户可能选择不属于自己的机构
   - **解决方案**：
     - **UserAccount 账号**：当匹配到多个机构时，不显示机构列表
     - 要求用户手动输入机构名称
     - 实现实时前缀匹配（不区分大小写）：
       - 用户输入机构名称的前几个字符
       - 系统根据输入的前缀实时匹配机构列表
       - 当只有 1 个机构匹配时：自动填充该机构的完整名称并自动选择
       - 当有多个机构匹配时：不弹出下拉列表，继续等待用户输入更多字符
       - 当用户停止输入（blur）时进行验证：
         - 如果输入不匹配任何机构 → 提示"机构名错误"
         - 如果输入匹配多个机构 → 提示"请输入更多字符以区分机构"
     - **Email/Phone 账号**：保持原有逻辑（显示机构列表供选择）
       - Email 和 Phone 通常与用户身份强绑定，风险较低
       - 用户通常知道自己的 Email/Phone 对应的机构
   - **优势**：
     - 防止恶意用户选择不属于自己的机构
     - 用户必须知道机构名称才能登录（增加安全性）
     - 通过前缀匹配提升用户体验（自动填充）
...
3. **智能填充**
   ...
   - 多个机构匹配时：
     - **Email/Phone**：显示选择列表
     - **UserAccount**：要求手动输入机构名称，实现实时前缀匹配和自动填充
...
2. 后端返回所有匹配的机构列表
3. **根据账号类型处理**：
   - **Email/Phone**：前端显示机构选择列表，用户选择目标机构
   - **UserAccount**：不显示机构列表，要求用户手动输入机构名称，通过前缀匹配自动填充
4. 提交登录时携带选中的机构 ID
...
2. **账号格式识别**
   - **Email**：包含 `@` 符号
   - **Phone**：纯数字或带格式（如 +1-234-567-8900），至少 7 位数字
   - **UserAccount（用户名）**：其他格式（不包含 `@` 且不是纯数字格式）
   - **前端实现**：使用 `detectAccountType()` 函数自动识别账号类型

3. **Hash 计算与账号规范化**
   - **UserAccount（user_account）**：
     - 数据库字段 `user_account` 有 CHECK 约束确保存储为小写：`CHECK (user_account = LOWER(user_account))`
     - 前端计算 hash 时：转换为小写，去除空格后计算 SHA-256
     - 后端使用 `user_account_hash` 字段进行匹配
   - **Email**：
     - 转换为小写（大小写不敏感，标准做法），去除空格后计算 SHA-256
     - 后端使用 `email_hash` 字段进行匹配
   - **Phone**：
     - 去除空格后计算 SHA-256（电话号码是数字，无需转换大小写）
     - 后端使用 `phone_hash` 字段进行匹配
